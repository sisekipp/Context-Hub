...
export function GraphqlSourceForm({ onClose, onCreated, onSaved, initialValue }: { onClose: () => void; onCreated?: (source: GraphqlBrowserSource) => void; onSaved?: () => void; initialValue?: GraphqlSourceInput }) {
  ...
  return <div className="rest-source-overlay" role="presentation">
    <form className="rest-source-dialog graphql-source-dialog" onSubmit={submit} aria-label="GraphQL data source">
      ...
      <div className="rest-pairs graphql-headers"><div className="rest-pairs-title"><span>Headers</span><button type="button" onClick={() => update({ headers: [...source.headers, { key: "", value: "" }] })}><Plus size={12}/> Add</button></div>{source.headers.map((header, index) => <div className="rest-pair" key={`graphql-header-${index}`}><input aria-label={`GraphQL header key ${index + 1}`} placeholder="Key" value={header.key} onChange={(event) => updateHeader(index, { key: event.target.value })}/><input type={/^(authorization|cookie|proxy-authorization|x-api-key)$/i.test(header.key) ? "password" : "text"} aria-label={`GraphQL header value ${index + 1}`} placeholder="Value" value={header.value} onChange={(event) => updateHeader(index, { value: event.target.value })}/><button type="button" aria-label={`Remove GraphQL header ${index + 1}`} onClick={() => update({ headers: source.headers.filter((_, headerIndex) => headerIndex !== index) })}><Trash2 size={12}/></button></div>)}{!source.headers.length && <small>No headers configured.</small>}</div>
      <div className="rest-security-note">Only the configured query and variables are sent. Sensitive headers are encrypted separately and only shown as masked placeholders.</div>
