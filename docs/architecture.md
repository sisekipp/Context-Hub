...
`IngestionService.Start` runs the current asynchronous worker path. It verifies the immutable ontology version and the source/mapping ownership, reads and checksum-validates the MinIO object, and converts JSON, NDJSON, or CSV input to shared Arrow batches. Every restricted DataFusion object plan in the mapping bundle runs over those batches; nodes are merged globally before cross-plan links are resolved. The worker expands list-valued link references into individual edges, applies Create/Skip/Error for missing link targets, validates the resulting types, and writes bounded node and edge batches through the same graph repository. Job transitions and counters are write-through persisted in ClickHouse. Browser uploads are unary and capped at 32 MiB until a multipart upload contract is introduced.
